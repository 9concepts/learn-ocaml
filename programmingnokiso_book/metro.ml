...
type eki_t = {
  ...
  temae_list : string list; (* 漢字の文字列のリスト *)
}

(* 12.2 *)
(* make_eki_list : ekimei_t list -> eki_t list *)
let rec make_eki_list = function
  | [] -> []
  | { kanji } :: rest ->
...
let shokika stations start_station_name =
  let init eki =
    match eki with
    | { namae } ->
        if namae = start_station_name then
          { namae; saitan_kyori = 0.; temae_list = [ namae ] }
        else eki
  in
  List.map init stations

(* let test_shokika = shokika global_eki_list "千駄木" *)

(* 12.4 *)
(* 要素をソート済み配列の適切な場所に挿入する *)
let rec insert compare item = function
  | [] -> [ item ]
  | head :: rest ->
      if compare head item < 0 then head :: insert compare item rest
      else item :: head :: rest
...
let rec insert_sort = function
  | [] -> []
  | head :: rest ->
      let compare_ekimei a b =
        match (a, b) with
        | { romaji = romaji_a }, { romaji = romaji_b } ->
            if romaji_a = romaji_b then 0
            else if romaji_a < romaji_b then -1
            else 1
      in
      insert compare_ekimei head (insert_sort rest)

(* let test_insert_sort = insert_sort global_ekimei_list *)

let rec uniq prev_kanji list =
  match list with
  | [] -> []
  | ({ kanji } as head) :: rest ->
      if kanji = prev_kanji then uniq kanji rest else head :: uniq kanji rest
...
(* let koushin1 p q =
   ...
   | ( {
         namae = p_namae;
         saitan_kyori = p_saitan_kyori;
         temae_list = p_temae_list;
       },
       { namae = q_namae; saitan_kyori = q_saitan_kyori } ) ->
       let rec update = function
         | [] -> q
         | { kiten; shuten; kyori } :: rest ->
             if
               (kiten = p_namae && shuten = q_namae)
               || (kiten = q_namae && shuten = p_namae)
             then
               if p_saitan_kyori +. kyori < q_saitan_kyori then
                 {
                   namae = q_namae;
                   saitan_kyori = p_saitan_kyori +. kyori;
                   temae_list = q_namae :: p_temae_list;
                 }
               else q
             else update rest
       in
       update global_ekikan_list *)

(* 13.7 *)
(* 直前に確定した駅 p (eki_t) と未確定の駅のリスト (eki_t list) を受け取り、最短距離を更新したリストを返す *)
(* let koushin p v = List.map (koushin1 p) v *)

(* let test_koushin =
     koushin
       { namae = "市ヶ谷"; saitan_kyori = 0.; temae_list = [ "市ヶ谷" ] }
       [
...
let koushin p v =
  let rec find_edge p q edges =
    ...
    | (_, _), [] -> None
    | ( ({ namae = p_namae }, { namae = q_namae }),
        ({ kiten; shuten } as edge) :: t ) ->
        if
          (kiten = p_namae && shuten = q_namae)
          || (kiten = q_namae && shuten = p_namae)
        then Some edge
  ...
  let koushin1 p q =
    ...
    | Some edge -> (
        match (p, q, edge) with
        | ( { saitan_kyori = p_saitan_kyori; temae_list = p_temae_list },
            { saitan_kyori = q_saitan_kyori; namae = q_namae },
            { kyori } ) ->
            if p_saitan_kyori +. kyori < q_saitan_kyori then
              {
                namae = q_namae;
